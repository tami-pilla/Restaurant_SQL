...
SELECT TOP 5 restaurant_name, restaurant_type, SUM(total_orders) as total_money_collected
FROM dataset_restaurant_clean
...
SELECT TOP 5 restaurant_name, restaurant_type, SUM(total_orders) as total_money_collected, meal_type
FROM dataset_restaurant_clean
...
SELECT TOP 5  member_id, SUM(total_orders) as total_orders
FROM dataset_restaurant_clean
GROUP BY member_id
...
SELECT top 5 date, hour, MAX(total_orders) as max_total_orders
FROM dataset_restaurant_clean
GROUP BY date, hour, total_orders
ORDER BY max_total_orders DESC

SELECT FORMAT(date,'MM') as mes , COUNT(total_orders) as max_total_orders
FROM dataset_restaurant_clean
GROUP BY  total_orders, date
ORDER BY date, COUNT(total_orders) DESC
...
SELECT  date, COUNT(date) as catidad
FROM dataset_restaurant_clean
